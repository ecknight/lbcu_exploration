...
searchMovebankStudies(x="longBilledCurlew", login=loginStored)

id.usgs <- getMovebankID("longBilledCurlew_USGS_ASC_argos", login = loginStored)
...
sens.wy <- getMovebankSensors(id.wy, login=loginStored) %>% 
  mutate(project="wy")
sens.usgs <- getMovebankSensors(id.usgs, login=loginStored) %>% 
  mutate(project="usgs")

sens <- rbind(sens.tx, sens.iw, sens.mn, sens.bc, sens.wy, sens.usgs) %>% 
...
dat.usgs <- getMovebankLocationData(id.usgs, sensorID=c(82798), login=loginStored, removeDuplicatedTimestamps=TRUE, includeOutliers=TRUE)
...
write.csv(dat.usgs, "Data/Movebank - Long-billed Curlew - USGS.csv", row.names = FALSE)
...
ref.usgs <- getMovebankReferenceTable(id.usgs, login=loginStored)
...
write.csv(ref.usgs, "Data/Movebank - Long-billed Curlew - USGS - Metadata.csv", row.names = FALSE)
